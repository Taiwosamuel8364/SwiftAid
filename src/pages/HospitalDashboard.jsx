...
import { calculateSeverity, getSeverityLevel } from "../utils/severityEngine";
  ...
  useEffect(() => {
    ...
    const onMessage = (topic, msg) => {
      ...
      if (score >= 60) {
        ...
        setAlerts((prev) => [enriched, ...prev.slice(0, 5)]);
      }
    };
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-100 via-blue-50 to-slate-200 p-6">
      ...
      {activeEmergency && (
        <div className="fixed top-6 right-6 w-96 bg-red-600 text-white p-5 rounded-2xl shadow-2xl animate-pulse z-50">
          ...
          <p className="mt-3 font-bold">Severity: {activeEmergency.severity}</p>

          <p className="mt-1 text-sm">Status: CRITICAL CONDITION</p>
        </div>
      )}
      ...
      <div className="bg-white/80 backdrop-blur-xl border border-white shadow-xl rounded-3xl p-6 mb-6">
        <div className="flex flex-col md:flex-row md:justify-between md:items-center">
          ...
          <div className="mt-4 md:mt-0 flex items-center gap-3">
            ...
            <span className="text-green-600 font-semibold">MQTT LIVE</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
        ...
        <div className="bg-white rounded-3xl shadow-lg p-6 border-l-4 border-red-500">
          ...
          <h2 className="text-3xl font-bold text-red-600">{alerts.length}</h2>
        </div>

        <div className="bg-white rounded-3xl shadow-lg p-6 border-l-4 border-emerald-500">
          <p className="text-slate-500">System Status</p>
          <h2 className="text-2xl font-bold text-emerald-600">OPERATIONAL</h2>
        </div>
      </div>
          ...
          {Object.values(patients).map((p) => (
            ...
            >
              ...
              <div className="space-y-2 text-slate-600">
                ...
                <p className="flex justify-between">
                  <span>📊 Severity</span>
                  <span className="font-bold text-slate-800">{p.severity}</span>
                </p>
              </div>
            </div>
          ))}
            ...
            {alerts.map((a, i) => (
              ...
              >
                <div className="flex justify-between">
                  <p className="font-bold text-red-700">{a.patientId}</p>

                  <p className="text-red-600 font-semibold">{a.level}</p>
                </div>
